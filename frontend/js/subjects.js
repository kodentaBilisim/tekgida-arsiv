...
function renderSubjects() {
    ...
    subjectsContainer.innerHTML = mainSubjects.map(subject => renderSubjectCard(subject)).join('');
}
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            ${hasChildren && isExpanded ? `
                <div class="border-t border-gray-100 bg-gray-50/50">
                    ${children.map(child => renderChildSubject(child)).join('')}
                </div>
            ` : ''}
        </div>
    `;
 ...
 */
function renderChildSubject(subject) {
    const colorClasses = getColorClass(subject.code);
